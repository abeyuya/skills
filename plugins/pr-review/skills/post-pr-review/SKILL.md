...
`commit_id` は caller 側で PR の head SHA (`headRefOid`) を取得して渡すと、force-push / rebase で行ズレが起きた際の誤コメントを防げる (`run-pr-review` は `compose-review` が返した `commit_id` を優先し、無ければ Step 2 で CHANNEL に応じて `gh pr view --json headRefOid` または `mcp__github__pull_request_read` method=`get` から取得した値を流用する想定)。加えて **CI が「現在の head SHA に対するレビューか」を review の `commit_id` で判定する** 運用では、`commit_id` を渡さないと GitHub 側が投稿時点の最新 commit を採用するため照合が不確実になる。機械判定を前提にするなら caller は常に `COMMIT_ID` を渡すこと (詳細は「機械可読サマリ行」節の「CI 側の使い方」)。
...
- **head SHA に対するレビューかの判定** は review の `commit_id` を PR の head SHA と比較する (本 skill は `COMMIT_ID` が渡された場合のみ `commit_id` を送るため、機械判定を前提にするなら caller は常に `COMMIT_ID` を渡す。`run-pr-review` は `compose-review` が返した `commit_id` (無ければ Step 2 で取得した `headRefOid`) を常時転送するので、その経路なら常に付く)。`COMMIT_ID` を渡さないと GitHub 側が投稿時点の最新 commit を採用するため、force-push と競合したときに照合が不確実になる。本改修で `COMMIT_ID` まわりの挙動自体は変更していない。
