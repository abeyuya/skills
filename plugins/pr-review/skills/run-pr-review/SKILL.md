...
- caller の cwd と PR の所属リポジトリが異なるドッグフーディング系では、Step 3 の `REVIEW.md` / `AGENTS.md` / `CLAUDE.md` がローカルに存在しないことがある (cwd の repo と PR の repo が別)。その場合は `gh api repos/<OWNER>/<REPO>/contents/<path>` でリモートから取得して `Read` 相当に扱う (Step 3 の補足)。API レスポンスの `content` フィールドは **Base64 エンコードされているため、`python3 -c "import base64,sys; print(base64.b64decode(sys.stdin.read()).decode())"` 等でデコードしてからファイル内容として利用すること**。
...
- Step 7 の check run `name` (`pr-review (abeyuya/skills)`) は本リポジトリ由来であることを示す固定文言。**本 plugin を fork / 別 org で再配信する場合**、本 SKILL.md (Step 7-2 の JSON スキーマ例) と README の「Check Run 出力」セクションの両方を新しい owner/repo に書き換えること (動的解決はしない: 名乗るべき owner/repo が caller 側 / plugin origin / fork 元のいずれか曖昧なため)。
