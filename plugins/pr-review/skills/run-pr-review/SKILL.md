...
- `commits` の末尾要素 (`gh pr view ... --json commits` の最後の `oid`) を head SHA として控え、Step 6 で `post-pr-review` の `COMMIT_ID` 引数として常時転送する (`post-pr-review/SKILL.md` の入力欄に定義された任意フィールド。force-push / rebase で行ズレが起きた際の誤コメント防止に有効)。
