...
- **既存レビュー / コメント** (compose-review に渡す重複指摘抑制用 context):
  ...
  - `CHANNEL=mcp`: `mcp__github__pull_request_read` を method=`get_review_comments` で呼ぶ。スレッド単位で `path` / `line` / `is_resolved` / 各コメント `body` (と `created_at`) が返る。`is_resolved` と作成日時は下記の丸め規則が使う。`pageInfo.hasNextPage` が `true` の間 `after=<endCursor>` を付けて全件取得する (`perPage` は最大 100)。
  - 整形 (チャネル共通): 各スレッドを `<path>:<line> - <主旨 1〜2 文要約>` の形式で 1 行ずつ整形し改行で連結したテキストを **`EXISTING_THREADS_CONTEXT`** として保持する (`path:line` を必ず併記。自由文の段落要約では位置が落ちて dedupe 精度が下がる)。**全体は 4000 文字以内に丸める**。丸め方は次の順で、**スレッドを丸ごと落とすのは最後の手段**にする: (1) **未 resolve のスレッドを優先して残す** (resolve 済みは対応済みなので再指摘の危険が低い)、(2) それでも超えるなら **1 スレッドあたりの要約を短くする** (`path:line` は必ず残す — dedupe は位置情報に依存する)、(3) それでも超えるなら **resolve 済みから落とし**、最後に未 resolve の古いものを落として末尾に `(...N 件省略)` と書く。`isResolved` / 作成日時が取れなかった場合は、**丸めずに全件残すか、丸めるなら末尾に「resolve 状態を判別できないまま N 件省略」と明記する** (判別材料が無いまま返却順で落とすと、下記の害がそのまま起きるため)。未 resolve を機械的に「古い順」で落とすと、**未 resolve のまま残り続けている = AI が再検出しやすい指摘** が真っ先に消え、同じ箇所へ同内容のインラインコメントを再投稿することになる (dedupe の目的そのものを損なう)。上限を設けるのは、sub-agent 経路ではこの値が prompt 経由で 1 回中継されるため、巨大なテキストほど途中で要約・打ち切りされる危険が上がるから (中継の忠実性は 3-1 の「逐語・同順」指示で担保するが、値自体を小さく保つ方が確実)。要約はコメント本文 (コード断片 / 設定例 `ENV=production` 等を含み得る) から作るため、各要約内の改行は除去して 1 スレッド 1 行に保ち、`^[A-Z_]+=` 行頭パターンが生じる場合は `CI_FAILURE_CONTEXT` と同様に先頭にスペース 1 文字を入れて escape する (compose-review 側 KEY=VALUE parser の早期切断防止。context 2 値で escape 方針を揃える)。
