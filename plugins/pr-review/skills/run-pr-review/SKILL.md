...
caller から渡されていればそれを使う。未指定なら `git remote get-url origin` の URL から抽出する (SSH 形式 `git@github.com:owner/repo.git` と HTTPS 形式 `https://github.com/owner/repo.git` の両対応: `git remote get-url origin | sed -E 's#\.git$##; s#.*[:/]([^/]+/[^/]+)$#\1#'`。先に末尾 `.git` を除去してから最後の 2 セグメントを取る 2 段構成にする。1 段で `(\.git)?` を末尾任意にすると貪欲マッチで `repo.git` ごと拾い `.git` が残る。この抽出は本 skill が正典を持つ)。gh が使える環境では `gh repo view --json nameWithOwner -q .nameWithOwner` を補助に使ってもよい。
...
- **PR メタ情報** (title / body / head ref / head SHA / base ref):
  ...
  - head SHA (`headRefOid`) を控え、**Step 3 で `compose-review` の `COMMIT_ID` 入力として渡す**。**Step 4 で `post-pr-review` へ渡す `COMMIT_ID` は `compose-review` が返した `commit_id` を使う** (force-push / rebase での行ズレによる誤コメント防止)。`compose-review` は fetch した実 head と突き合わせて `commit_id` を確定する契約なので、Step 2 時点の `headRefOid` で上書きすると、**その後に force-push が起きた回に「PR に存在しない commit」を anchor にして投稿し、Review が 422 で丸ごと失敗する / position がずれて別行に付く**。`headRefOid` は `commit_id` が欠落していた場合の defensive fallback としてのみ使う (Step 3「戻り値の扱い」参照)。`baseRefName` (PR の base ブランチ名) も控え、Step 3 で `compose-review` の `BASE_BRANCH` として転送する (非 default base の PR で compose-review が git 主経路の base を default branch に誤推定し差分範囲がズレるのを防ぐ)。
