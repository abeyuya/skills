...
- **`mode` が `"pr"` でない、または JSON として読めない (壊れている / 必須フィールド `body`・`event`・`comments` の欠落) 場合** → 整合性エラーとして Step 4 / 5 は実行せず停止し、Step 6 で「compose-review 戻り値が想定形式でなかった」旨を caller に報告する (壊れた入力のまま post-pr-review へ進めない)。
