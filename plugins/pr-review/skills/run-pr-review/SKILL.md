...
- **既存レビュー / コメント** (compose-review に渡す重複指摘抑制用 context):
  ...
  - 整形 (チャネル共通): 各スレッドを `<path>:<line> - <主旨 1〜2 文要約>` の形式で 1 行ずつ整形し改行で連結したテキストを **`EXISTING_THREADS_CONTEXT`** として保持する (`path:line` を必ず併記。自由文の段落要約では位置が落ちて dedupe 精度が下がる)。**全体は 4000 文字以内に丸める**。丸め方は次の順で、**未 resolve のスレッドを落とすのは最後の手段**にする:
    ...
    1. **resolve 済みのスレッドを先に落とす** (既に対応済みなので、落としても再指摘の害が小さい)。
...
- **長文 value (`EXISTING_THREADS_CONTEXT` / `CI_FAILURE_CONTEXT`) は Step 2 で丸めた内容を 1 文字も変えずに転記する**。sub-agent 経路ではこの値が LLM を 1 回余分に経由するため、**要約・打ち切り・整形は禁止**と prompt に明示する。これらが落ちると、既存スレッドの dedupe が効かず解決済み指摘が再投稿され、CI 文脈が落ちれば `[must]` 昇格が発火せず `must=0` のまま required status check を通過する。Step 2 で両値に文字数上限を設けているのは、この中継を現実的にするため。
