...
- `external_review` は **両モードで必ず含める** (5-2 の結末の機械可読な記録。算出規則は 5-2 の「外部レビュー結果の記録」)。キーは 8 つ固定:
  ...
  - `reason`: 未併用 / 縮退 (`inline` / `partial` / `empty` / `verify_degraded` / `omitted > 0`) の理由 1 行 (5-5 の開示文と同旨)。**加えて、`PRIOR_CODE_REVIEW` が渡されたのに不採用にした回もここに理由を入れる** (5-2 解決順 1 参照。この場合 `mode` は解決順 2 の実際の値のまま = 縮退していなくても `reason` が非 null になりうる)。上記いずれにも該当せず正常に併用できた場合は `null`。
