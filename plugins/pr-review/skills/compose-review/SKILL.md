...
- **PR モード**: **git 主経路** — Step 1 で退避した SHA を使い `git diff <BASE_SHA>...<HEAD_SHA>` (三点記法 = merge-base 基準で base 進行を除外。GitHub の "Files changed" と一致) を差分ソースにする。head/base の object は Step 1 で read-only fetch 済みなので `gh` は不要。ローカルの作業ツリー・ローカルブランチは一切変えない (「守ること」の read-only fetch 例外)。git 経路では出力打ち切りが起きないため truncation 検知 / ファイル単位の追い読みは不要。
  ...
  - **差分を取る前に `BASE_SHA` / `HEAD_SHA` が両方とも非空であることを確認する**。`BASE_SHA` が空だと `git diff ...<HEAD_SHA>` が三点記法の省略側を `HEAD` で補って **fatal にならず成功し**、cwd の HEAD 次第で空差分になる (指摘ゼロのレビューを黙って投稿する)。この状態は Step 3 の祖先 `REVIEW.md` 探索と 5-4 の発火条件 (どちらも同じ `--name-only`) も同様に汚染するので、**どちらか一方でも空なら Step 1 の SHA 解決失敗として「失敗時」に従い error 停止する** (`gh` 経路に切り替えて続けない。`gh` からは base の SHA を取れず、head だけ入れ替えると差分範囲と anchor の整合が崩れるため)。
