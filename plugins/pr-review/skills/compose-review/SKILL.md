...
- **PR モード**: **本文も見出し探索も `git show <HEAD_SHA>:<path>` を正典にする** (`<HEAD_SHA>` は Step 1 で確定した head SHA)。候補を優先順に 1 つずつ `git show <HEAD_SHA>:<path>` で取り、fatal が **path 不在** (`fatal: path '<path>' does not exist in '<HEAD_SHA>'`) なら次の候補へ進む。fatal が **object 不在** (`fatal: invalid object name` / `bad object`) なら候補不在ではなく Step 1 の head materialize が済んでいない状態なので、次の候補へ進まず「失敗時」に従い `{"error":"..."}` を書き出して停止する (cwd の `Read` フォールバックは無いため、ここで黙って続けると 4 候補すべて「不在」に見え、方針なし・`escalate: false` のまま投稿されてしまう)。Step 1 で `refs/pull/<PR_NUMBER>/head` を fetch 済みなのでその object から直接読める (cross-repo でも Step 1 で explicit URL から fetch 済みなら読める。`gh` は不要で、下記の任意の補助に限る)。
