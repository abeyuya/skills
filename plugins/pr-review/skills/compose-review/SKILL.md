...
- `PRIOR_CODE_REVIEW`: caller が **自身のコンテキストで既に得ている** 手動 `/code-review` の findings を転送するための **1 行 JSON 文字列** (任意)。形式は `{"target":"<その code-review が対象にした範囲の表現。ブランチ名 / ref range / PR 番号 / uncommitted 差分の別など、caller が観測できたまま>","head":"<その時点の head SHA。caller が特定できなければ null>","base":"<その code-review が比較ベースにしていた ref / SHA。分からなければ null>","findings":[{"file":"…","line":N,"summary":"…","failure_scenario":"…","verdict":"CONFIRMED"|"PLAUSIBLE"|null}, …]}`。`verdict` は `code-review` が finding ごとに返す検証結果で、**取れるなら必ず転送する** (5-2 のラベル付与が使う。取れない / 出力に無い場合は `null` か省略)。5-2 の解決順 1 (`code-review`) の入力になる。**本 skill が sub-agent として起動された回は caller のセッションコンテキストが見えないため、この引数が無ければ解決順 1 は成立しない** (直接呼びの回もコンテキスト上の findings に頼らず本引数を正典とする。詳細は 5-2 解決順 1)。物理的な改行を含めてはならない (`KEY=VALUE` parser が壊れるため)。**JSON 文字列内の改行は `\n` にエスケープすれば 1 物理行に収まる**ので、`failure_scenario` が複数行でも転送できる (caller が「改行があれば諦める」と字句どおり解釈してこの経路を不発にしないよう、エスケープ可能であることを明示する)。エスケープしても 1 行が過大になる規模のときだけ caller は転送を諦めて行ごと省略する。
  - **レビュー対象が一致するかの判定は本 skill の責務**。レビュー範囲 (PR モードの `<BASE_SHA>...<HEAD_SHA>`、ローカルモードの `diff_mode` とベース) を確定するのは Step 1 = 本 skill であり、caller はそれを知らない。したがって caller には「観測できた `target` / `head` をそのまま添えて転送する」ことだけを求め、採否は本 skill が 5-2 解決順 1 で決める (caller 側の足切りは「明らかに別 PR / 別ブランチを対象にしたと分かる場合は転送しない」程度でよい)。
...
- **解決順**:
  1. **`PRIOR_CODE_REVIEW` が渡されていれば、まずそれを 1 の入力として評価する** (下記「例外 (手動併用)」。**この経路は Skill ツールから `code-review` を呼べるかにも Agent ツールの有無にも依存しない** — findings は既に caller が持っており、本 skill は採否を判定するだけ。sub-agent で Agent ツールが提示されない回でもここは成立しうるので、下の呼び出し条件だけを見て「1 は不成立」と早合点しない。採否判定で不成立になった場合も、その事実は開示契約の対象)。渡されていない場合に限り、`code-review` (Claude Code 組み込み) が当セッションで **Skill ツールから実際に呼び出せて**、**かつ** Agent/Task ツールが当コンテキストで利用可能なら → これを使う (`code-review` は内部で Agent ツールによる finder/verifier の fan-out を行うため Agent ツールが必要)。呼び出し可能性の判定は下記「`code-review` の呼び出し可能性判定」に従う。
...
- **`code-review` の呼び出し可能性判定**: `code-review` は skill 定義の frontmatter に `disable-model-invocation: true` を持つため、**多くの Claude Code 環境ではモデルから Skill ツール経由で呼び出せない**。CLI の Skill ツール検証段階で `Skill code-review cannot be used with Skill tool due to disable-model-invocation` として拒否され、モデルに提示される available-skills 一覧からも除外される。この制約は settings.json のオプトインや `permissions.allow` では解除できない (検証が権限判定より前段のため)。判定と分岐:
  ...
  - **例外 (手動併用)**: ユーザーが同一セッションで先に `/code-review` を手動実行していた場合、その findings は **caller (orchestrator) が `PRIOR_CODE_REVIEW` 引数として転送してくる**。渡されていれば `code-review` を改めて呼ばず、**その `findings[]` を 1 の結果として採用** して下記「正規化」に流す (`external_review.skill` は `"code-review"`、`mode` は `"external"`。この運用は plugin README の「外部レビューの手動併用」に記載)。
    - **コンテキストに残っている findings を直接拾う経路には依存しない**。本 skill は sub-agent として起動されうるが、その場合 caller のセッションコンテキストは見えないため「先に実行された `/code-review` の出力」は本 skill からは観測できない。検出と転送は caller の責務とし、本 skill は `PRIOR_CODE_REVIEW` だけを正典とする (現在コンテキストで直接呼ばれた回も同じ扱いにして、経路によって挙動が変わらないようにする)。`PRIOR_CODE_REVIEW` が無ければこの手動併用の例外は使えないので、解決順 1 冒頭の `code-review` 呼び出し条件 (Skill ツールから呼べるか + Agent ツールがあるか) の判定に戻る (それも不可なら 2 へ進む)。
    - **採用前に、その findings が本 step のレビュー対象に対して実行されたものかを判定する** (`/code-review` は target を任意に取れるため、セッション前半に別ブランチ / 数コミット前の状態で実行された findings が本 PR の外部レビュー結果として紛れ込みうる。`external_review` には対象範囲も実行時刻も含まれないので事後には区別できない)。判定は Step 1 で確定した本 step のモードごとに行う:
      - **PR モード**: `PRIOR_CODE_REVIEW.head` が Step 1 の `HEAD_SHA` と **一致すれば採用**。不一致 / `head` が `null` or 欠落なら **1 は不成立** として 2 へ進む。投稿を伴う経路なので SHA 一致だけを根拠にし、`target` 文字列の解釈で緩めない。
      - **ローカル `commit` モード**: `head` が `git rev-parse HEAD` と一致すれば採用。**`head` が `null` / 欠落なら不成立** (PR モードと同じ扱い)。caller は「その `/code-review` が見ていた head を特定できないとき `null` にする」契約なので、`null` を `target` 文字列の一致だけで救うと **古い findings を弾くという `head` の存在意義が消える** (commit を積んだ後でも `target` は「同じブランチ」のままなので必ず一致してしまう)。`target` は `head` が一致した上での追加の矛盾チェックに使う (別ブランチ / 別 PR を指していれば `head` 一致でも不成立)。**さらに、`base` (または `target` から読み取れる比較ベース) が渡されていて Step 1 で解決したベースブランチと異なる回は不成立にする** — ローカル `commit` モードの差分は `<base>...HEAD` なので、同一ブランチ・同一 head でもベースが違えば別範囲になる (例: 先に `main` ベースで `/code-review` を実行 → 今回 `BASE_BRANCH=develop`)。**`base` が `null` / 未提供の回は head 一致を主ガードとして採用してよい** (ベース不明を不成立にするとこの経路が構造的に成立しなくなるため。ベース違いは head も変わることが多く、head 一致で大半は排除できる)。
      - **ローカル `staged` / `worktree` モード**: uncommitted 差分は SHA で同定できないため `head` では判定しない。**`target` が uncommitted 差分 (`staged` / `worktree` 相当) を指していると明示的に読め、かつ `/code-review` 実行後に対象ファイルが編集されていないと caller 側で確認できている (その旨が `target` / `reason` から読める) ときだけ採用**する。ブランチ名や ref range を指しているなら対象が違うので不成立。**uncommitted 差分は SHA で同定できず、`/code-review` の後にファイルを直してから本 skill を呼ぶのが自然な使い方なので、staleness を確認できない回は不成立に倒す** (`commit` モードが `head` で防いでいるのと同じ事故 — 修正済み指摘の再掲と行 anchor のずれ — が、このモードでは何のガードも無いまま起きる)。**`target` が空 / 省略されている回は「情報が無い」ので不成立に倒す** (下記の原則どおり) — 省略を「uncommitted の証拠」と読むと、別範囲・別時点で実行された findings がそのまま採用されて再掲される。
      - 判定に必要な情報が無く、どちらとも言えない場合は **不成立** に倒す (古い findings を混ぜるより、`scan-diff-findings` に落ちる方が安全)。
    - **`PRIOR_CODE_REVIEW` が渡されたのに不採用にした回は、それを記録・開示する** (必須)。解決順 2 が正常成功すると `external_review` は `{"skill":"scan-diff-findings","mode":"agent","reason":null}` になり、5-5 の開示も不要ケースになるため、**ユーザーが先に `/code-review` を回した findings が捨てられた事実がどこにも残らない**。そこでこの回は `external_review.reason` に理由を 1 行入れ (例: `PRIOR_CODE_REVIEW は head 不一致 (stale) のため不採用。scan-diff-findings で代替`)、5-5 の `## 総合判断` 末尾にも 1 文添える (例: `先行実行された code-review の findings は対象 head が異なるため採用せず、scan-diff-findings を併用した。`)。**`findings` が空配列だったために不成立にした回は、開示文を「指摘が 0 件だった」旨に書き分ける** (例: `先行実行された code-review は指摘 0 件だったため、scan-diff-findings を併用した。`) — 「採用しなかった」という文言だけだと、実在した指摘を捨てたかのような事実誤認になる。**`reason` が非 null でも `mode` は解決順 2 の実際の値のままにする** (縮退の表現と混同しない)。
    - `PRIOR_CODE_REVIEW` が JSON として parse できない / `findings` が配列でない場合も 1 は不成立として 2 へ進む (error にはしない)。
    - **`findings` が空配列の回も 1 は不成立** として 2 へ進む (`scan-diff-findings` 経路の「`findings` が 1 件以上あるのに…」というガードと対称にする)。空配列で 1 を成立扱いにすると、`scan-diff-findings` が一度も呼ばれないまま `external_review` が `{"skill":"code-review","mode":"external","findings":0}` = 「正常に併用できた」として記録され、**外部レビューが実質 0 系統なのに縮退として現れない**。さらに `verdict` が空集合 = 「全件 null」に該当してしまい、外部指摘が 1 件も無いのに `verify_degraded: true` と「外部由来の指摘は未検証」の開示が付く (下記 verdict 規則も `findings` が 1 件以上ある回にのみ適用する)。
    - **`verdict` をラベル付与に反映する**: `verdict` が `"PLAUSIBLE"` (= `code-review` 側で確証が取れなかった) の finding は、`scan-diff-findings` の `confidence: "unverified"` と **同じ扱い**にする — `failure_scenario` を差分から自分で追認できなければ 1 段下げる (`[must]` → `[should]`、`[should]` → `[nit]`)。`"CONFIRMED"` はそのまま。これを省くと未検証の指摘が下げ処理なしに `[must]` として `label_counts.must` を押し上げる。
    - **ただし `verdict` が全件 `null` / 欠落の回は 1 段下げを機械適用しない** (`scan-diff-findings` 経路の `verify_degraded` に課しているガードと同じ)。`/code-review` の `verdict` は verify pass が走った回にしか出ないため、機械適用すると **全件降格 → `label_counts.must` が 0** になり、`must=0` を required check にしている運用で **マージが不当に通る** (`mode="external"` は本来 `verify_degraded` を `null` にする枠なので、下記の例外規則で `true` を立てない限り CI 側からも検知できない)。この回は 5-1 と同じ基準で `failure_scenario` を自分で追認し、追認できたものは元の重大度、できないものだけ 1 段下げる。
    - 一部だけ `verdict` を持つ回は、持たない finding を上記「全件 null」と同じ自己追認で扱い、`"PLAUSIBLE"` は 1 段下げ規則をそのまま適用する。
    - **`findings` が 1 件以上あり、その `verdict` が全件 `null` / 欠落の回は `external_review.verify_degraded` を `true` にし、5-5 で開示する** (必須)。`mode="external"` は本来 `verify_degraded: null` だが (Step 6)、この回は「adversarial verify を通っていない指摘を採用した」ことが判っているので `null` にしてはならない — `null` のままだと、`scan-diff-findings` 経路なら `verify_degraded=true` + 開示必須になる同じ状況が、`code-review` 経路では機械可読にも人間向けにも記録されずに通ってしまう。**1 件でも `verdict` を持つ回は `null`** (`mode="external"` の既定に戻す。`"PLAUSIBLE"` は 1 段下げで表現される)。値の正典は Step 6 の `verify_degraded` 項なので、両者を食い違わせない。
...
- **外部レビュー未併用 / 独立性縮退の開示 (必須)**: 5-2 の結末に応じて `## 総合判断` の末尾に 1 文を記載する。
  ...
  - **`PRIOR_CODE_REVIEW` を採用したが verdict が全件無かった** (`external_review.verify_degraded = true` かつ `mode="external"`) → 文例: `外部レビューは先行実行された code-review の findings を採用したが、finding ごとの検証結果 (verdict) が得られなかったため、外部由来の指摘は未検証。`
  ...
  - **`PRIOR_CODE_REVIEW` を渡されたが不採用にした** (5-2 解決順 1 の判定で stale / 対象不一致 / parse 不能) → 文例: `先行実行された code-review の findings は対象 head が異なるため採用せず、scan-diff-findings を併用した。` (解決順 2 が正常成功していてもこの 1 文は必須。ユーザーが意図して回した外部レビューを捨てた事実を残すため)
  ...
  - **正常に併用できた** (`external_review.reason` が `null` かつ `fanout.mode="agent"` かつ verify 段も正常 かつ `omitted == 0` / `fanout` を返さない外部スキルを併用した `mode="external"`) → 開示文は不要 (どのスキルを併用したかの記載は任意。機械可読な記録は `external_review` が担う)。**`"external"` はそれ自体は縮退ではない** — `code-review` / Codex `/review` 等が `fanout` 相当の内訳を返さないだけなので、開示対象に含めない。**ただし `verify_degraded = true` になった `mode="external"` (`verdict` が全件無かった回) は上記のとおり開示必須**。
...
- `external_review` は **両モードで必ず含める** (5-2 の結末の機械可読な記録。算出規則は 5-2 の「外部レビュー結果の記録」)。キーは 8 つ固定:
  ...
  - `verify_degraded`: `findings` が 1 件以上あるのに外部スキルの `fanout.verified == 0` なら `true` (adversarial verify が丸ごと機能しなかった)。それ以外は `false`。値が判断できない外部スキル (`mode="external"`) と **未併用 (`skill == "none"`)** は `null` (未併用時に `false` を出すと「verify 済みで健全」と読めてしまうため)。**例外: `PRIOR_CODE_REVIEW` を採用し、その `findings[]` が 1 件以上あって `verdict` が全件 `null` / 欠落だった回は `mode="external"` でも `true` にする** (verify を通っていない指摘を採用したと判っているケース。5-2 解決順 1 参照)。**`verdict` を 1 件でも持つ回はこの例外に当たらないので `null` のまま** (`false` にはしない — `mode="external"` では verify 段の全体像が分からないため)。
  ...
  - `reason`: 未併用 / 縮退 (`inline` / `partial` / `empty` / `verify_degraded` / `omitted > 0`) の理由 1 行 (5-5 の開示文と同旨)。**加えて、`PRIOR_CODE_REVIEW` が渡されたのに不採用にした回もここに理由を入れる** (5-2 解決順 1 参照。この場合 `mode` は解決順 2 の実際の値のまま = 縮退していなくても `reason` が非 null になりうる)。
...
- **実行コンテキスト (sub-agent / 現在コンテキスト直接呼び) によって手順を変えない**。本 skill は両経路で呼ばれる (「caller 向け呼び出し契約」参照)。経路の判別を試みたり、判別できないことを理由に 5-2 をスキップしたりしない。経路差を吸収するのは caller 側の責務で、本 skill から見た差は `PRIOR_CODE_REVIEW` が渡ってくるかどうかだけ。
