...
- **PR モード**: **git 主経路** — Step 1 で退避した SHA を使い `git diff <BASE_SHA>...<HEAD_SHA>` (三点記法 = merge-base 基準で base 進行を除外。GitHub の "Files changed" と一致) を差分ソースにする。head/base の object は Step 1 で read-only fetch 済みなので `gh` は不要。ローカルの作業ツリー・ローカルブランチは一切変えない (「守ること」の read-only fetch 例外)。git 経路では出力打ち切りが起きないため truncation 検知 / ファイル単位の追い読みは不要。
  ...
  - **差分を取る前に `BASE_SHA` / `HEAD_SHA` が両方とも非空であることを確認する**。`BASE_SHA` が空だと `git diff ...<HEAD_SHA>` が三点記法の省略側を `HEAD` で補って **fatal にならず成功し**、cwd の HEAD 次第で空差分になる (指摘ゼロのレビューを黙って投稿する)。空なら Step 1 の base SHA 解決失敗として「失敗時」に従い error 停止する。
  - **`git diff <BASE_SHA>...<HEAD_SHA>` が fatal を返したら、理由を問わず空差分と解釈してはならない** (object がローカルに無い / shallow checkout で共通祖先が無く `fatal: no merge base` 等) (「対象差分なし」で指摘ゼロのレビューを黙って投稿することになる)。上記の `gh` 補助経路に切り替え、それも使えなければ **差分取得不能として「失敗時」に従い `{"error":"..."}` を書き出して停止する** (空差分の分岐へ落とさない)。`gh pr diff` は **その時点の PR head** の差分を返すので、この経路に切り替えた回は `gh pr view <PR_NUMBER> --repo <OWNER>/<REPO> --json headRefOid -q .headRefOid` で head を取り直し、出力 `commit_id` をその値に更新する (Step 1 の `HEAD_SHA` のままだとコメント anchor が差分とズレる)。
...
- **判定基準の自己回避を防ぐ (PR モードで必須)**: 基準は **レビュー対象 PR が書き換えられるファイル** にあるため、作成者が同一 PR で基準セクションを削除する / 文言を狭める / **上位候補ファイルを新設して既存の基準を shadowing する** (Step 3 は「最初に見つかった 1 つだけ」を読むため、基準を持つ `AGENTS.md` の手前に基準の無い `REVIEW.md` を追加すれば基準なし扱いになる) と、明示的な「判定するな」という指示を書かずに判定を `escalate: false` へ落とせる。したがって **Step 4 の差分が root の 4 候補または任意階層の `REVIEW.md` を追加 / 変更 / 削除している回** (`git diff <BASE_SHA>...<HEAD_SHA> --name-only` に含まれる) は次の 2 つを行う:
  ...
  - **base 側が読めないとき**: base 側の走査前に `git cat-file -e <BASE_SHA>^{commit}` で base object の存在を確認する (Step 3 の head 側と同じ理由: object 不在でも `git show` の fatal は path 不在と同じ文面になる)。本ルールの発火条件の判定 (`git diff <BASE_SHA>...<HEAD_SHA> --name-only`) は base / head 両方の object を要するので、これが fatal になる場合も「差分に含まれない」と読まず **どちらかの object が未取得** として扱い、一覧は `gh pr diff --name-only <PR_NUMBER> --repo <OWNER>/<REPO>` (または `gh api --paginate repos/<OWNER>/<REPO>/pulls/<PR_NUMBER>/files` の `filename`) から取り直す (head は取得済みで base だけ未取得の回もここに含む) (head 側が未取得なら Step 3 の degrade が先に走っているはずだが、ここでも同じ扱いにして沈黙で `escalate: false` に落ちるのを防ぐ)。未取得なら **まず Step 3 と同じ `gh api -H "Accept: application/vnd.github.raw" repos/<OWNER>/<REPO>/contents/<path>?ref=<BASE_SHA>` に degrade する (raw の Accept ヘッダは Step 3 と同じ理由で必須。既定の JSON は Base64 なので、そのまま見出し `grep` をかけると base 側が常に「基準なし」に見え、rule 2 の削除・shadowing 検知が発火しない)** (head 側の補助経路と対)。それも使えず base 側をどの経路でも読めないときだけ、次のように分ける:
    - **head 側に `エスカレーション基準` 見出しがある回**: `escalate: true` とし `reasons[]` に `エスカレーション基準の突き合わせ不能: base object (<BASE_SHA>) が未取得` を 1 行入れる (基準を持つ caller なので、迷ったらエスカレーションする方向。誤検知しても PR は止まらない)。
    - **head 側に見出しが無い回**: 従来どおり `{"escalate": false, "reasons": []}` とする。ただし「見出しが無い」は Step 3 が読んだ最初の 1 候補だけでは結論しない — **root の 4 候補すべてと、差分が触っている `REVIEW.md` とその祖先** を head 側で確認してから false に倒す (上位候補を新設して下位の基準を shadowing する回を、base 側が読めないことを理由に素通しさせないため)。ここで `escalate: true` に倒すと、基準を持たない caller の全 PR に `AI-REVIEW-ESCALATE` 行が付き「基準を持たない caller の出力は従来と完全に同一」という後方互換の前提が崩れる (発火条件を判定できない回も含め全 PR が該当してしまうため)。
