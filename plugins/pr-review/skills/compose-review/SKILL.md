...
- 見出し探索は **取得した全文に対して、見出し行に限定した** `grep -nE '^#{1,6} .*エスカレーション基準'` で行う (PR モードは `git show <HEAD_SHA>:<path> | grep -nE '^#{1,6} .*エスカレーション基準'`、ローカルモードは `grep -nE '^#{1,6} .*エスカレーション基準' <path>`)。`head` を通した出力に `grep` をかけない。見出し行に限定するのは、本文中の言及 (例: 「エスカレーション基準はまだ定めていない」) を見出しと取り違えて 5-4 の判定に入らないため (opt-in の条件は見出しの存在のみ)。
- 「見出しが無い」と結論する前に、**取得した行数がファイルの総行数と一致している** ことを確認する。総行数は `grep -c '' <path>` (PR モードは `git show <HEAD_SHA>:<path> | grep -c ''`) で数える。`wc -l` は改行の個数なので末尾改行が無いファイルで 1 少なく出て、全文を取得しても不一致になり再取得を繰り返すため使わない。一致しなければ全文を取得し直してから探索する。
