...
- **PR モード**: **本文も見出し探索も `git show <HEAD_SHA>:<path>` を正典にする** (`<HEAD_SHA>` は Step 1 で確定した head SHA)。候補を優先順に 1 つずつ `git show <HEAD_SHA>:<path>` で取り、fatal (path が無い) なら次の候補へ進む。Step 1 で `refs/pull/<PR_NUMBER>/head` を fetch 済みなのでその object から直接読める (cross-repo でも Step 1 で explicit URL から fetch 済みなら読める。`gh` は不要で、下記の任意の補助に限る)。
