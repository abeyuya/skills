...
- `MAX_INLINE_COMMENTS` が指定された件数を指摘候補が超える場合は、`/pr-review-style-reference` の重要度序列 (`[must]` > `[should]` > `[nit]` / `[q]`) で上位を残す。省略した指摘がある場合は総括 (`## 総括`) に「省略件数 + ラベル別内訳」を 1 文添える (`/pr-review-style-reference` の引数仕様に準拠)。
