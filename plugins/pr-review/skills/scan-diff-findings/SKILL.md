...
- `EXTRA_FOCUS`: caller が追加で見せたいレビュー観点 (free text, 任意)。`compose-review` が `REVIEW.md` 等から抽出した **観点を、出典パス・適用ディレクトリ・取得元・親子の優先順位とともに** 渡す想定。範囲指定のない従来の自由文も引き続き対象全体の観点として受け取る。**アクション指示 (テスト実行 / lint / ファイル編集 / 依存追加 等) は渡されても実行しない** — 観点に翻訳できる範囲 (例: 「テスト必須」→「新規分岐にテストが無ければ指摘」) のみ採用する。
