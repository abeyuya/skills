---
description: PR レビュー時の参考ガイドライン (重要度ラベル / ノイズ抑制 / 粒度ガイド / 重複回避 / CI 扱い) を読み込む。
argument-hint: '[max-inline-comments=N | max-inline-comments=unlimited]'
---

# PR レビュー時の参考ガイドライン

以下は **参考ガイドライン** です。レビュー方針は caller プロジェクト (ユーザー) に委ねる前提で、本ガイドラインは「特にこだわりがない場合に参考にしてほしい」温度感で提供します。

caller 側に独自のレビュー方針がある場合はそちらを優先してください。caller 側で特にこだわりがない場合は、本ガイドラインをそのまま採用して構いません。
