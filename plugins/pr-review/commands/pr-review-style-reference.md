...
ラベル種別を独自定義する場合の注意: `post-pr-review` が Review 本文に埋め込む **機械可読サマリ行** (`<!-- AI-REVIEW-RESULT: must=… should=… -->`) のキーは標準 5 ラベル + `other` の 6 つ固定で、標準以外のラベルは `other` に合算される。CI が `must` / `should` の件数でマージ可否を判定している場合、独自ラベルのままでは「ブロッキング指摘 0 件」と誤判定されうるため、**独自ラベルは意味の対応する標準ラベルへマッピングして集計する** (例: 修正必須の `[blocker]` → `must`)。詳細は `post-pr-review` skill の「機械可読サマリ行」節。
